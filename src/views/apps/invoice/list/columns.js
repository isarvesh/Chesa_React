...
const renderClient = row => {
  ...
  if (row.avatar.length) {
    ...
    return <Avatar color={color} className='me-50' content={row.client ? row.client.name : 'Sam'} initials />
  }
}
...
export const columns = [
  ...
  },
  {
    ...
    cell: row => {
      const name = row.client ? row.client.name : 'Sam',
        email = row.client ? row.client.companyEmail : 'Sam@email.com'
      return (
